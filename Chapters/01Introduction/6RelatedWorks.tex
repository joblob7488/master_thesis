...
\textcolor{red}{må kanskje KORT forklare hva forskjellene mellom Vanilla, Convolutional og Coalesced, da det blir relevant i diskusjonen}
...
Extending this to spatial data, the first hardware accelerator for the Convolutional TM (CTM) was presented in 2023 \textcolor{red}{CITE TUNHEIM 2023}. The design targeted binary image classification and supported both inference and on-device learning, and evaluted on the NoisyXOR dataset (Section \ref{sec:other_datasets}). It achieved a throughput 13.3 times higher than a software implementation running on a 2.7 GHz CPU. The work also identified the quality of the random number generator as a critical factor in achieving accuracy comparable to software, finding a 16-bit LFSR to be the optimal configuration. 

More recently, two hardware accelerators for the Convolutional Coalesced TM (ConvCoTM) were presented in 2025. The first is an FPGA implementation targeting $28 \times 28$ grayscale images across ten output classes, achieving 134k classifications per second and $13.3\,\mu$J energy per classification (EPC) on a Xilinx Zynq ZU7EV while supporting full on-device learning \textcolor{red}{CITE TUNHEIM 2025 FPGA}. The design was evaluated on MNIST, Fashion-MNIST and Kuzushiji-MNIST (Section~\ref{sec:other_datasets}), maintaining competitive accuracy across all three datasets.The second is a 65 nm ASIC inference-only implementation of the same architecture, achieving 60.3k classifications per second at just 8.6 nJ EPC, one of the lowest reported for TM hardware \textcolor{red}{CITE TUNHEIM 2025 ASIC}. Together, these designs demonstrate how dedicated TM hardware can deliver high energy efficiency and competitive accuracy for edge deployment. 

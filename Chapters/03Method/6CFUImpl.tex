...
internal logic of the instruction \cite{Nolting2024Datasheet}. The full VHDL
implementation is given in Appendix \ref{app:cfu1vhdl}.
...
By performing the vote accumulation and threshold clamping in hardware, this design removes the per-clause and per-class scoring arithmetic from the software loop entirely. Like in the first design, each operation incurs the fixed three cycle CFU latency, however due to the design now being sequential rather than combinatorial, we also get one additional clock cycle of latency.

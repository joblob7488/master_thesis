...
// 28x28 pixels = 784 features + 783 spaces + 1 label + 1 newline + 1 null terminator -> approx 1600 chars per line
#define MAX_LINE_LENGTH 1600
...
{	
	...
	if (mc_tm_save("MNIST.model", mc_tm) != 0) {
		fprintf(stderr, "Failed to save model\n");
		return 1;
	}
	clock_t end_save = clock();
	double time_used = ((double) (end_save - start_save)) / CLOCKS_PER_SEC;
	printf("\nModel saved to MNIST.model\nSaving Time: %.1f s\n", (time_used));
